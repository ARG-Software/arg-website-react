...
export function getRateLimitConfig(env: EnvSource = process.env): RateLimitConfig {
  return {
    perMinute: getNumberEnv(env, 'RAG_ASK_RATE_LIMIT_PER_MINUTE', DEFAULT_PER_MINUTE),
    perDay: getNumberEnv(env, 'RAG_ASK_RATE_LIMIT_PER_DAY', DEFAULT_PER_DAY),
    globalDaily: getNumberEnv(env, 'RAG_ASK_GLOBAL_RATE_LIMIT_PER_DAY', DEFAULT_GLOBAL_DAILY),
    salt: env.RAG_ASK_RATE_LIMIT_SALT || 'arg-ask-rate-limit',
  };
}
