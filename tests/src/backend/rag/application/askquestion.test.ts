...
import { EmbeddingQuotaExceededError } from '../../../../../src/backend/rag/application/ports/providererrors.js';
import { createFakeAnswerProvider as createAnswerProvider } from '../fakes/fakeanswer.provider.js';
import { createFakeEmbeddingProvider as createEmbeddingProvider } from '../fakes/fakeembedding.provider.js';
...
import { loadFirstPartySources } from '../../../../../src/backend/rag/infrastructure/ingestion/loaders/loadfirstpartysources.js';
...
test('CRM questions use business-system wording instead of unsupported-stack wording', async () => {
  const result = await askQuestion({
    ...
    config,
    readRepository: createSupabase({}).repository,
    answerProvider: createAnswerProvider('Does ARG Software work with CRM integrations?', {
      plan: { mode: 'direct_evidence', entity: 'ARG Software', subject: 'CRM integrations' },
    }),
    embeddingProvider: createEmbeddingProvider(() => [[0.1, 0.2]]),
    fallbackEmbeddingProvider: createEmbeddingProvider(() => [[0.1, 0.2]]),
  });
  ...
  assert.doesNotMatch(result.answer, /usual or preferred stack/iu);
  assert.match(result.answer, /integrating an existing CRM|building a custom CRM/iu);
  assert.match(result.answer, /HubSpot|Salesforce|Pipedrive|Zoho/u);
});

test('custom CRM questions mention product architecture scope', async () => {
  const result = await askQuestion({
    question: 'Can you create a CRM?',
    config,
    readRepository: createSupabase({}).repository,
    answerProvider: createAnswerProvider('Can ARG Software create a CRM?', {
      plan: { mode: 'direct_evidence', entity: 'ARG Software', subject: 'custom CRM' },
    }),
    embeddingProvider: createEmbeddingProvider(() => [[0.1, 0.2]]),
    fallbackEmbeddingProvider: createEmbeddingProvider(() => [[0.1, 0.2]]),
  });
  ...
  assert.doesNotMatch(result.answer, /usual or preferred stack/iu);
  assert.match(result.answer, /custom CRM/iu);
  assert.match(result.answer, /data model|permissions|reporting|automations/iu);
});

test('CMS questions use content-platform wording instead of unsupported-stack wording', async () => {
  const result = await askQuestion({
    question: 'Do you support CMS?',
    config,
    readRepository: createSupabase({}).repository,
    answerProvider: createAnswerProvider('Does ARG Software support CMS?', {
      plan: { mode: 'direct_evidence', entity: 'ARG Software', subject: 'CMS' },
    }),
    embeddingProvider: createEmbeddingProvider(() => [[0.1, 0.2]]),
    fallbackEmbeddingProvider: createEmbeddingProvider(() => [[0.1, 0.2]]),
  });
  ...
  assert.doesNotMatch(result.answer, /usual or preferred stack/iu);
  assert.match(result.answer, /headless CMS|custom admin|back-office/iu);
  assert.match(result.answer, /Strapi|Contentful|Sanity|WordPress/u);
});

test('Strapi CRM questions clarify that Strapi is a CMS, not a CRM', async () => {
  const result = await askQuestion({
    question: 'Is Strapi a CRM?',
    config,
    readRepository: createSupabase({}).repository,
    answerProvider: createAnswerProvider('Is Strapi a CRM?', {
      plan: { mode: 'direct_evidence', entity: 'ARG Software', subject: 'Strapi CRM' },
    }),
    embeddingProvider: createEmbeddingProvider(() => [[0.1, 0.2]]),
    fallbackEmbeddingProvider: createEmbeddingProvider(() => [[0.1, 0.2]]),
  });
  ...
  assert.doesNotMatch(result.answer, /usual or preferred stack/iu);
  assert.match(result.answer, /Strapi is not a CRM/u);
  assert.match(result.answer, /headless CMS/u);
});

test('ERP questions use operations-system wording instead of unsupported-stack wording', async () => {
  const result = await askQuestion({
    question: 'Can you build an ERP?',
    config,
    readRepository: createSupabase({}).repository,
    answerProvider: createAnswerProvider('Can ARG Software build an ERP?', {
      plan: { mode: 'direct_evidence', entity: 'ARG Software', subject: 'ERP' },
    }),
    embeddingProvider: createEmbeddingProvider(() => [[0.1, 0.2]]),
    fallbackEmbeddingProvider: createEmbeddingProvider(() => [[0.1, 0.2]]),
  });
  ...
  assert.doesNotMatch(result.answer, /usual or preferred stack/iu);
  assert.match(result.answer, /existing ERP|custom internal operations software/iu);
  assert.match(result.answer, /Odoo|SAP|NetSuite|Microsoft Dynamics/u);
});
