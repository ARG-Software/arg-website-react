...
export function parseArticleMarkdown(raw) {
...
export function parseArticleMetadata(raw) {
  const { meta, body } = parseFrontmatter(raw);
  return extractMetadata(meta, body);
}

export function parseArticleWithContent(raw) {
  const { meta, body } = parseFrontmatter(raw);
...
export function loadArticles() {
  return Object.entries(mdModules)
    .map(([path, raw]) => {
      const article = parseArticleMarkdown(raw);
...
export function loadArticlesMetadata() {
  return Object.entries(mdModules)
    .map(([path, raw]) => {
      const article = parseArticleMetadata(raw);
      const match = path.match(/\/(\d+)-/);
      article._order = match ? parseInt(match[1], 10) : 0;
      return article;
    })
...
export function loadArticlesWithContent() {
  return Object.entries(mdModules)
    .map(([path, raw]) => {
      const article = parseArticleWithContent(raw);
      const match = path.match(/\/(\d+)-/);
      article._order = match ? parseInt(match[1], 10) : 0;
      return article;
    })
