...
export function getReferenceRank(metadata: RagSourceMetadata | null | undefined): number {
  const rank = metadata?.reference_rank;
  return typeof rank === 'number' ? rank : Number.POSITIVE_INFINITY;
}

export function getRequestedProjectCount(value: string, matchCount: number): number {
