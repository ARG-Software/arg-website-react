import type { IChatMessage } from './chatmessage.types.js';
import type { IPageContext } from './pagecontext.types.js';

export interface IHomepageSectionScope {
  sourceKey: string;
}

export interface IPageContextLookups {
  getHomepageSectionScope(sectionId: string): IHomepageSectionScope | null;
  getProjectNameBySlug(slug: string): string | undefined;
  getStaticPageSourceKeys(pathname: string): string[] | undefined;
}
...
export function normalizePageContext(
  ...
  const normalizedActiveSection =
    normalizedPathname === '/' && typeof activeSection === 'string'
      ? lookups.getHomepageSectionScope(activeSection)
      : null;
  ...
  const staticSourceKeys = lookups.getStaticPageSourceKeys(normalizedStaticPath);
  ...
  return {
    ...
    ...(projectSlug
      ? {
          ...
          projectSlug,
          projectName: lookups.getProjectNameBySlug(projectSlug) ?? getTitleEntity(normalizedTitle, projectSlug),
