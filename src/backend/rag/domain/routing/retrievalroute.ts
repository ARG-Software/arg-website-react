...
export function resolveRetrievalRoute(
  ...
  const projectEntity = resolveProjectEntity(routeText, plan.entity, knownProjectNames);
  ...
  return createRoute('company_services', DIRECT_EVIDENCE_SOURCE_TYPES, plan);
}
...
function resolveProjectEntity(routeText: string, entity: string, knownProjectNames: string[]): string {
  ...
  const match = knownProjectNames.find(projectName =>
