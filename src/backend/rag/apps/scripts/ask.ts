...
try {
  ...
  if (retrieveOnly) {
    const contexts = await container.assistant.retrieveRelevantChunksUseCase.execute({
      question,
      messages,
      pageContext,
    });
  ...
  const result = await container.assistant.askAssistantQuestionUseCase.execute({
    question,
    messages,
    pageContext,
  });
