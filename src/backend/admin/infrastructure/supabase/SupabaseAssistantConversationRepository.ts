import type { IAssistantConversationRepository } from '../../application/ports/IAssistantConversationRepository.js';
...
export class SupabaseAssistantConversationRepository implements IAssistantConversationRepository {
  ...
  async upsert(logRecord) {
    const row = toDatabaseRow(logRecord, this.securityCodec);
    ...
    return toConversationRecord(data, this.securityCodec);
  }
...
function toDatabaseRow(logRecord, securityCodec) {
  const encryptedPayload = securityCodec.encryptAssistantConversationPayload(logRecord.payload);
...
function toConversationRecord(row, securityCodec) {
  return {
    id: row.id,
    publicConversationId: row.public_conversation_id,
    payload: securityCodec.decryptAssistantConversationPayload(row),
    messageCount: row.message_count,
    pagePath: row.page_path,
    language: row.language,
    lastMessageAt: row.last_message_at,
