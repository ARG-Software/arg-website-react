export function toOutreachRecord(row, securityCodec) {
  const protectedFields = securityCodec.decryptOutreachFields(row);
...
export function toOutreachDatabaseRow(record, securityCodec) {
  const encryptedFields = securityCodec.encryptOutreachFields({
