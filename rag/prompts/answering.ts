...
export function buildIntentPrompt(companyName: string): string {
  return [
    ...
    `rag_question means questions about ${companyName}, its services, projects, team, founder experience, rates, budgets, estimates, partners, careers, blog posts, external profiles, contact options, legal pages, or follow-ups about prior ${companyName}-related answers. It also includes general technical questions where the visitor may benefit from ${companyName}'s published technical insights, and technical service enquiries asking whether ${companyName} can assess or deliver work.`,
...
export function buildInsufficientContextPrompt(companyName: string, responseLanguage: string): string {
  return [
    ...
    buildResponseLanguageInstruction(responseLanguage),
    'Say briefly that we do not have enough information in the available ARG Software context to answer.',
...
export function buildIntentFallbackPrompt(
  ...
  return [
    ...
    intent === 'small_talk'
      ...
      : `Politely redirect the user to ${companyName} website topics such as services, projects, careers, partners, blog posts, or contact options. If the question is a technical service enquiry, say that we need to understand the requirements before assessing it and invite the visitor to book a meeting or contact us.`,
...
export function buildUserPrompt(question: string, contexts: RetrievedContext[]): string {
  const contextText = contexts
    .map((context, index) => {
      ...
      return `[${index + 1}] Approved ARG knowledge: ${citation}\n${context.content}`;
    })
    .join('\n\n');
